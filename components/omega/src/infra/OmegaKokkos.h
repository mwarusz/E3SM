...
using Scratch1DReal =
    Kokkos::View<Real *, MemLayout, ScratchMemSpace, MemoryUnmanaged>;
...
constexpr int OMEGA_REDUCE_VECTORSIZE = 64;

#else

constexpr int OMEGA_TEAMSIZE          = 1;
constexpr int OMEGA_VECTORSIZE        = 1;
constexpr int OMEGA_REDUCE_VECTORSIZE = 1;
#endif

template <class T>
KOKKOS_FUNCTION auto getScratch(const TeamMember &Team, int Size) {
   if constexpr (OMEGA_VECTORSIZE == 1) {
      return T(Team.team_scratch(0), Size);
   } else {
      return T(Team.thread_scratch(0), Size);
   }
}
...
inline void parallelForOuter(const std::string &Label,
   ...
   if constexpr (OMEGA_VECTORSIZE == 1) {
      ...
      Policy.set_scratch_size(0, Kokkos::PerTeam(ScratchSize));
      Kokkos::parallel_for(
      ...
      Policy.set_scratch_size(0, Kokkos::PerTeam(OMEGA_TEAMSIZE * ScratchSize));
      Kokkos::parallel_for(
          Label, Policy, KOKKOS_LAMBDA(const TeamMember &Team) {
             const int TeamId   = Team.league_rank();
...
inline void parallelForOuter(const int (&UpperBounds)[N], F &&Functor,
                             int ScratchSize = 0) {
   parallelForOuter("", UpperBounds, std::forward<F>(Functor), ScratchSize);
}
...
inline void parallelReduceOuter(const std::string &Label,
   ...
   auto Policy = TeamPolicy(LinBound, 1, OMEGA_REDUCE_VECTORSIZE);
