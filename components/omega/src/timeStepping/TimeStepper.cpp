...
TimeStepper *TimeStepper::create(
   ...
   OMEGA_REQUIRE(
       InTend,
       "Null Tendencies pointer in TimeStepper::create with Name = {}", InName);
   OMEGA_REQUIRE(
       InAuxState,
       "Null AuxiliaryState pointer in TimeStepper::create with Name = {}",
       InName);
   OMEGA_REQUIRE(
       InMesh, "Null HorzMesh pointer in TimeStepper::create with Name = {}",
       InName);
   OMEGA_REQUIRE(
       InVCoord,
       "Null VertCoord pointer in TimeStepper::create with Name = {}", InName);
   OMEGA_REQUIRE(
       InMeshHalo, "Null Halo pointer in TimeStepper::create with Name = {}",
       InName);
...
void TimeStepper::init1(const TimeInitParams &TimeParams) {
   ...
   OMEGA_REQUIRE(OmegaConfig,
                 "Null OmegaConfig pointer in TimeStepper::init1");
   Config TimeIntConfig("TimeIntegration");
   Err = OmegaConfig->get(TimeIntConfig);
   CHECK_ERROR_ABORT(Err, "TimeIntegration group not found in Config");
...
void TimeStepper::init2() {
   ...
   OMEGA_REQUIRE(DefMesh, "Null default HorzMesh pointer in TimeStepper::init2");
