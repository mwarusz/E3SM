...
int initState(const Array2DReal &LayerThickCell,
   ...
   Err += setScalar(
       ...
       FVertex, Geom, Mesh, VCoord, OnVertex);
...
int testKineticAuxVars(const Array2DReal &LayerThicknessCell,
   ...
   Err += setScalar(
       ...
       ExactKineticEnergyCell, Geom, Mesh, VCoord, OnCell, ExchangeHalos::No);
   ...
   Err += setScalar(
       ...
       ExactVelocityDivCell, Geom, Mesh, VCoord, OnCell, ExchangeHalos::No);
   ...
   Err += computeErrors(KineticEnergyErrors, NumKineticEnergyCell,
                        ExactKineticEnergyCell, Mesh, VCoord, OnCell);
   ...
   Err += computeErrors(VelocityDivErrors, NumVelocityDivCell,
                        ExactVelocityDivCell, Mesh, VCoord, OnCell);
...
int testWindForcingAuxVars(Real RTol) {
   ...
   Err += setVectorEdge(
       ...
       ExactNormalStressEdge, EdgeComponent::Normal, Geom, Mesh, VCoord,
       ExchangeHalos::No);
   ...
   Err += setScalar(
       ...
       WindForcingAux.ZonalStressCell, Geom, Mesh, VCoord, OnCell);

   Err += setScalar(
       KOKKOS_LAMBDA(Real X, Real Y) { return Setup.windStressY(X, Y); },
       WindForcingAux.MeridStressCell, Geom, Mesh, VCoord, OnCell);
   ...
   Err += computeErrors(NormalStressErrors, NumNormalStressEdge,
                        ExactNormalStressEdge, Mesh, VCoord, OnEdge);
...
int testLayerThicknessAuxVars(const Array2DReal &LayerThickCell,
   ...
   Err += setScalar(
       KOKKOS_LAMBDA(Real X, Real Y) { return Setup.layerThickness(X, Y); },
       ExactThickEdge, Geom, Mesh, VCoord, OnEdge, ExchangeHalos::No);
...
int testVorticityAuxVars(const Array2DReal &LayerThickCell,
   ...
   Err += setScalar(
       ...
       ExactRelVortVertex, Geom, Mesh, VCoord, OnVertex, ExchangeHalos::No);
   ...
   Err += setScalar(
       ...
       ExactNormRelVortVertex, Geom, Mesh, VCoord, OnVertex, ExchangeHalos::No);
   ...
   Err += setScalar(
       ...
       ExactNormPlanetVortVertex, Geom, Mesh, VCoord, OnVertex,
       ExchangeHalos::No);
   ...
   Err += computeErrors(RelVortVertexErrors, NumRelVortVertex,
                        ExactRelVortVertex, Mesh, VCoord, OnVertex);
   ...
   Err += computeErrors(NormRelVortVertexErrors, NumNormRelVortVertex,
                        ExactNormRelVortVertex, Mesh, VCoord, OnVertex);
   Err +=
   ...
   Err += computeErrors(NormPlanetVortVertexErrors, NumNormPlanetVortVertex,
                        ExactNormPlanetVortVertex, Mesh, VCoord, OnVertex);
   ...
   Err += setScalar(
       ...
       ExactNormRelVortEdge, Geom, Mesh, VCoord, OnEdge, ExchangeHalos::No);
   ...
   Err += setScalar(
       ...
       ExactNormPlanetVortEdge, Geom, Mesh, VCoord, OnEdge, ExchangeHalos::No);
   ...
   Err += computeErrors(NormRelVortEdgeErrors, NumNormRelVortEdge,
                        ExactNormRelVortEdge, Mesh, VCoord, OnEdge);
   ...
   Err += computeErrors(NormPlanetVortEdgeErrors, NumNormPlanetVortEdge,
                        ExactNormPlanetVortEdge, Mesh, VCoord, OnEdge);
...
int testVelocityDel2AuxVars(Real RTol) {
   ...
   Err += setScalar(
       KOKKOS_LAMBDA(Real X, Real Y) { return Setup.divergence(X, Y); },
       ExactVelocityDivCell, Geom, Mesh, VCoord, OnCell);

   Array2DReal ExactRelVortVertex("ExactRelVortVertex", Mesh->NVerticesSize,
                                  NVertLayers);
   Err += setScalar(
       KOKKOS_LAMBDA(Real X, Real Y) { return Setup.relativeVorticity(X, Y); },
       ExactRelVortVertex, Geom, Mesh, VCoord, OnVertex);
   ...
   Err += setVectorEdge(
       ...
       ExactDel2Edge, EdgeComponent::Normal, Geom, Mesh, VCoord,
       ExchangeHalos::No);
   ...
   Err += computeErrors(Del2Errors, NumDel2Edge, ExactDel2Edge, Mesh, VCoord,
                        OnEdge);
   ...
   Err += setScalar(
       ...
       ExactDel2DivCell, Geom, Mesh, VCoord, OnCell, ExchangeHalos::No);
   ...
   Err += setScalar(
       ...
       ExactDel2RelVortVertex, Geom, Mesh, VCoord, OnVertex, ExchangeHalos::No);
   ...
   Err += computeErrors(Del2RelVortErrors, NumDel2RelVortVertex,
                        ExactDel2RelVortVertex, Mesh, VCoord, OnVertex);
...
int testTracerAuxVars(const Array2DReal &LayerThickCell,
   ...
   Err += setScalar(
       ...
       TracersOnCell, Geom, Mesh, VCoord, OnCell);

   Array2DReal LayerThickEdge("LayerThickEdge", Mesh->NEdgesSize, NVertLayers);
   Err += setScalar(
       KOKKOS_LAMBDA(Real X, Real Y) { return Setup.layerThickness(X, Y); },
       LayerThickEdge, Geom, Mesh, VCoord, OnEdge);
   ...
   Err += setScalar(
       ...
       ExactHTrEdge, Geom, Mesh, VCoord, OnEdge, ExchangeHalos::No);
   ...
   Err += computeErrors(HTracerErrors, NumHTrEdge, ExactHTrEdge, Mesh, VCoord,
                        OnEdge);
   ...
   Err += setScalar(
       ...
       ExactDel2TrCell, Geom, Mesh, VCoord, OnCell, ExchangeHalos::No);
